...
import SomeReduxComponent from "./SomeReduxComponent";
...
describe("Redux testing", () => {
  it("checks initial state is equal to 0", () => {
    const { getByRole } = renderWithRedux(<SomeReduxComponent />);
    expect(getByRole("heading")).toHaveTextContent("0");
  });

  it("increments the counter through redux", () => {
    const { getByRole, getByText } = renderWithRedux(<SomeReduxComponent />, {
